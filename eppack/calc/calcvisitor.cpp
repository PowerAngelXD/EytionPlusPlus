...
void cvisitor::visitor::visitNumber(epplex::Token* token){
    if(token->content.find(".") != token->content.npos)
        ins.push_back({"__PUSH__", cenv::calc_unit("__DECI__", atof(token->content.c_str())), token->line, token->column});
    else
        ins.push_back({"__PUSH__", cenv::calc_unit("__INT__", atoi(token->content.c_str())), token->line, token->column});
}
void cvisitor::visitor::visitString(epplex::Token* token){
    ...
    ins.push_back({"__PUSH__", cenv::calc_unit("__STRING__", result), token->line, token->column});
}
void cvisitor::visitor::visitChar(epplex::Token* token){
    ...
    ins.push_back({"__PUSH__", cenv::calc_unit("__CHAR__", ch), token->line, token->column});
}
void cvisitor::visitor::visitIden(east::IdentifierNode* node){
    ins.push_back({"__PUSH__", cenv::calc_unit("__NULL__", node->idens[0]->content), node->idens[0]->line, node->idens[0]->column}); // TODO: After updating the scope, it needs to be adapted to the scope
}
void cvisitor::visitor::visitAddOp(east::AddOpNode* node){
    ins.push_back({node->op->content, cenv::calc_unit(node->op->content, 0.0), node->op->line, node->op->column});
}
